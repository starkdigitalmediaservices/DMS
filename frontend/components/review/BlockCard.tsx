...
function CellEditor({ initial, label, onCommit, onCancel, onTab }: {
...
function TableBody({ block, editMode, canVerify, busy, filter, selected, onSelect, onHover, actions }: Props) {
  ...
  return (
    // `relative` keeps absolutely-positioned descendants (the sr-only header
    // label) inside this scroll container instead of widening the page.
          ...
          {visibleRows.map((row) => {
            ...
            return (
              <tr
                ...
                className={`group align-top border-b border-[#f0f0f0] ${rowSelected ? "bg-[#e8f0fe]" : ""} ${row.deleted ? "opacity-60" : ""}`}
              >
                <td className="px-1 py-1.5 text-[11px] text-[#5f6368]">
                  <button type="button" className="hover:underline" title={row.page ? `Row from page ${row.page}` : "Added row"}
                    onClick={() => onSelect({ blockId: block.id, rowId: row.id })}>
                    {rowIdx + 1}
                  </button>
                ...
                {row.cells.map((cell, col) => {
                  ...
                  const cellSelected = rowSelected && selected?.col === col;
                  return (
                    <td
                      key={col}
                      ...
                      className={`relative px-2 py-1.5 min-w-[5rem] ${
    ...
    >
      <header className="flex flex-wrap items-center gap-2 mb-2">
        <button type="button" onClick={() => onSelect({ blockId: block.id })}
          className="flex items-center gap-2 text-left" title={block.source_pages.length ? `Pages ${block.source_pages.join(", ")}` : undefined}>
        ...
        {block.source_pages.length > 0 && <span className="text-[11px] text-[#5f6368]">p. {block.source_pages.join(", ")}</span>}
